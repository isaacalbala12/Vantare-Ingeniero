#!/usr/bin/env python3
"""Night final - improved extractor + best prompts + 14 questions"""
import sys
...
def extract_mm(msg):
    """Extract from MiniMax - answer is after thinking, take last data line"""
    ...
    # Find lines with actual data (numbers or key words)
    data_lines = []
    for line in lines:
        if len(line) < 100 and (any(c.isdigit() for c in line) or any(w in line.lower() for w in ['entrar', 'boxes', 'urgente', 'gestionar', 'atacar', 'defender', 'si', 'no', 'critico', 'normal', 'aumentando', 'acelerando'])):
...
def extract_sf(msg):
    """Extract from StepFun - clean content, remove Wait filler"""
    content = (msg.get('content') or msg.get('reasoning') or '').strip()
    content = re.sub(r'<think>.*?</think>', '', content, flags=re.DOTALL).strip()
    
    # Remove "Wait, wait" filler and similar
    ...
    # Take the last substantive line
...
async def call(client, url, model, key, sys, content, is_sf):
    payload = {'model': model, 'messages': [{'role': 'system', 'content': sys}, {'role': 'user', 'content': content}], 'max_tokens': 100, 'temperature': 0.1}
...
async def run():
    ...
    PROMPTS = [
        ...
        ('VD1', 'You are a race engineer. State your answer in 1-3 words. No explanation. No sentences. Example: P3 L10 42.3 / Entrar boxes.'),
