#!/usr/bin/env python3
"""Full 22-question test with SYNC to verify reproducibility"""
import sys
...
def run_sync(name, sys_prompt, run_i):
    print(f'\n--- {name} Run {run_i+1}/3 ---')
    total_sc = 0
    by_level = {}
    ...
    for i, item in enumerate(ORIG_Q):
        ...
        payload = {'model': 'step-3.7-flash', 'messages': [{'role': 'system', 'content': sys_prompt}, {'role': 'user', 'content': content}], 'max_tokens': 500, 'temperature': 0.1, 'thinking': {'type': 'off'}}
        ...
        if r.status_code == 200:
            ...
            print(f'[{i+1:2d}] ERROR {r.status_code}')
...
for run_i in range(3):
    run_sync('ORIG', ORIG_SYS, run_i)
    run_sync('NEW', NEW_SYS, run_i)
