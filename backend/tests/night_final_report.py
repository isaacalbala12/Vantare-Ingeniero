#!/usr/bin/env python3
"""Final definitive comparison - 3 runs, 22 questions, SYNC, for both MM and SF"""
import sys
...
def run_model(model_name, api_key, base_url, model_id, sys_prompt, extractor, is_sf, runs=3):
    h = {'Authorization': f'Bearer {api_key}', 'Content-Type': 'application/json'}
    all_results = []
    
    for run_i in range(runs):
        print(f'  {model_name} Run {run_i+1}/{runs}...')
        total = 0
        by_level = {}
        ...
        for item in ORIG_Q:
            ...
            payload = {'model': model_id, 'messages': [{'role': 'system', 'content': sys_prompt}, {'role': 'user', 'content': content}], 'max_tokens': 500, 'temperature': 0.1}
            if is_sf: payload['thinking'] = {'type': 'off'}
            
            r = httpx.post(base_url + '/chat/completions', headers=h, json=payload, timeout=120)
            
            if r.status_code == 200:
                text = extractor(r.json()['choices'][0]['message'])
                s = sc(text, kw)
                total += s
                if lvl not in by_level: by_level[lvl] = []
                by_level[lvl].append(s)
        
        pct = total / len(ORIG_Q) * 100
        lvl_str = ' '.join(f'L{lv}:{sum(by_level[lv])/len(by_level[lv])*100:.0f}' for lv in sorted(by_level.keys()))
        all_results.append(pct)
        print(f'    {pct:.1f}% | {lvl_str}')
    
    avg = sum(all_results) / len(all_results)
    return all_results, avg

print('='*70)
print('DEFINITIVE BENCHMARK: Original vs NEW2 Prompt (22 questions, 3 runs)')
print('='*70)
print()
print('Running MM-ORIG...')
mm_orig_runs, mm_orig_avg = run_model('MM-ORIG', M, 'https://api.minimaxi.chat/v1', 'MiniMax-M2.7', ORIG_SYS, extract_mm, False, 3)
print()
print('Running MM-NEW...')
mm_new_runs, mm_new_avg = run_model('MM-NEW', M, 'https://api.minimaxi.chat/v1', 'MiniMax-M2.7', NEW_SYS, extract_mm, False, 3)
print()
print('Running SF-ORIG...')
sf_orig_runs, sf_orig_avg = run_model('SF-ORIG', S, 'https://api.stepfun.ai/step_plan/v1', 'step-3.7-flash', ORIG_SYS, extract_sf, True, 3)
print()
print('Running SF-NEW...')
sf_new_runs, sf_new_avg = run_model('SF-NEW', S, 'https://api.stepfun.ai/step_plan/v1', 'step-3.7-flash', NEW_SYS, extract_sf, True, 3)

print()
print('='*70)
print('FINAL RESULTS')
print('='*70)
print(f'MiniMax-M2.7 ORIG: {mm_orig_avg:.1f}% (runs: {mm_orig_runs})')
print(f'MiniMax-M2.7 NEW:  {mm_new_avg:.1f}% (runs: {mm_new_runs}) -> DIFF: {mm_new_avg-mm_orig_avg:+.1f}%')
print()
print(f'StepFun-3.7 ORIG:  {sf_orig_avg:.1f}% (runs: {sf_orig_runs})')
print(f'StepFun-3.7 NEW:   {sf_new_avg:.1f}% (runs: {sf_new_runs}) -> DIFF: {sf_new_avg-sf_orig_avg:+.1f}%')
print()
print(f'COMBO ORIG: {(mm_orig_avg+sf_orig_avg)/2:.1f}%')
print(f'COMBO NEW:  {(mm_new_avg+sf_new_avg)/2:.1f}%')
print()
print(f'WINNING PROMPT: NEW')
print(f'"{NEW_SYS}"')
