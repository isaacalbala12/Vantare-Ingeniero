#!/usr/bin/env python3
"""Verify MM with 22 questions, sync, 3 runs"""
import sys
...
for run_i in range(3):
    for name, sys_prompt in [('ORIG', ORIG_SYS), ('NEW', NEW_SYS)]:
        ...
        for item in ORIG_Q:
            ...
            payload = {'model': 'MiniMax-M2.7', 'messages': [{'role': 'system', 'content': sys_prompt}, {'role': 'user', 'content': content}], 'max_tokens': 500, 'temperature': 0.1}
        ...
        print(f'{name} R{run_i+1}: {pct:.1f}% | {lvl_str}')

print('\nResults:')
print('MM-ORIG R1-R3: see above')
print('MM-NEW R1-R3: see above')
