#!/usr/bin/env python3
"""Iterate on SF prompts to beat 47.3%"""
import sys
...
PROMPTS = [
    ...
    # More concise
    ('V1', 'Short direct answer. Numbers only for data. Short action for advice. Be very concise. Examples: P3 L10 42.3 | Entrar boxes.'),
    # With more action verbs
    ('V2', 'Race engineer: Data → extract numbers. Advice → short verb. Examples: P3 L10 F42.3 | Entrar boxes urgente. Very short answers.'),
    # More like original benchmark
    ('V3', 'Eres ingeniero de carrera. Maximo 2-3 frases. Estilo radio. Para preguntas de datos: numeros. Para accion: verbo corto.'),
    # Even shorter
    ('V4', 'Race engineer. Short answer. Max 3 words. Examples: P3 L10 42.3 | Entrar boxes.'),
]
...
async def call(client, url, model, key, sys, content):
    ...
    for pname, sys_prompt in PROMPTS:
        ...
        async with httpx.AsyncClient() as client:
            ...
            for item in Q:
                ...
                tasks.append(call(client, 'https://api.stepfun.ai/step_plan/v1', 'step-3.7-flash', S, sys_prompt, content))
            msgs = await asyncio.gather(*tasks)
            
            for i, item in enumerate(Q):
                kw = item[3]
                text = extract_sf(msgs[i]) if msgs[i] else ''
                scores.append(sc(text, kw))
        
        pct = sum(scores) / len(Q) * 100
        results.append((pname, pct, sys_prompt))
        print(f'{pname}: {pct:.1f}%')
    
    print(f'\nDone in {time.time()-t0:.1f}s')
    results.sort(key=lambda x: x[1], reverse=True)
    print('\n--- RANKING ---')
    for n, pct, p in results:
        print(f'{n}: {pct:.1f}%')
    print(f'\nBEST: {results[0][0]} = {results[0][1]:.1f}%')
    print(f'PROMPT: {results[0][2]}')
