#!/usr/bin/env python3
"""Verify NEW2 and test StepFun-3.5 + temperature variations"""
import sys
...
BEST_PROMPT = 'Race engineer response: Data questions -> numbers only. Advice questions -> short action. Be concise. Example data: P3 L10 F42.3. Example advice: Entrar boxes urgente.'
...
async def call(client, url, model, key, sys, content, is_sf, temp=0.1):
...
async def run():
    configs = [
        ('MM-t01', 'minimaxi', 'MiniMax-M2.7', M, False, 0.1),
        ('MM-t03', 'minimaxi', 'MiniMax-M2.7', M, False, 0.3),
        ('SF37-t01', 'stepfun', 'step-3.7-flash', S, True, 0.1),
        ('SF37-t03', 'stepfun', 'step-3.7-flash', S, True, 0.3),
        ('SF35-t01', 'stepfun', 'step-3.5-flash', S, True, 0.1),
    ]
    ...
    for cfg_name, api_name, model, key, is_sf, temp in configs:
        ...
        async with httpx.AsyncClient() as client:
            ...
            for item in Q:
                ...
                tasks.append(call(client, base_url, model, key, BEST_PROMPT, content, is_sf, temp))
