#!/usr/bin/env python3
"""Final definitive test: MM and SF with best prompts, 3 runs"""
import sys
...
async def run():
    configs = [
        ('MM-NEW2-t01', 'minimaxi', 'MiniMax-M2.7', M, 'Race engineer response: Data questions -> numbers only. Advice questions -> short action. Be concise. Example data: P3 L10 F42.3. Example advice: Entrar boxes urgente.', False, 0.1),
        ('SF-NEW2-t01', 'stepfun', 'step-3.7-flash', S, 'Race engineer response: Data questions -> numbers only. Advice questions -> short action. Be concise. Example data: P3 L10 F42.3. Example advice: Entrar boxes urgente.', True, 0.1),
    ]
    ...
    print('=== FINAL RESULTS (3 runs average) ===')
    for cfg_name in all_results:
        runs = all_results[cfg_name]
        avg = sum(runs) / len(runs)
        print(f'{cfg_name}: {avg:.1f}% (runs: {[f"{r:.1f}" for r in runs]})')
    
    mm_avg = sum(all_results['MM-NEW2-t01']) / 3
    sf_avg = sum(all_results['SF-NEW2-t01']) / 3
    combined = (mm_avg + sf_avg) / 2
    print(f'\n*** COMBINED: {combined:.1f}% (MM={mm_avg:.1f}% + SF={sf_avg:.1f}%) ***')
