...
async def test_race_start_goes_immediate_not_batch():
    ...
    eng.apply_runtime_config({"verbosityLevel": "normal", "engineerEnabled": True})
    ...
    loop.on_frame(base, now=1.0)
    loop.on_frame({**base, "lap_number": 2, "standing_position": 5}, now=8.0)

    alerts = [m for m in sent if isinstance(m, AlertMessage)]
    assert any("salida" in a.message.lower() or "vamos" in a.message.lower() for a in alerts)
