#!/usr/bin/env python3
"""Benchmark V2 para modelos locales: qwen3.5-4b, smollm3-3b-128k, granite-4.1-8b"""
import sys, os, httpx, json, time, re
sys.stdout.reconfigure(encoding='utf-8')

BASE_URL = "http://192.168.1.41:1234/v1"
...
MODELS = [
    ...
    ('smollm3-3b-128k', 'Smollm3 3B (Local)'),
    ('granite-4.1-8b', 'Granite 4.1 8B (Local)'),
]


def extract_response(response_data):
    """Extract response from LLM message dict or string.
    
    Handles multiple formats:
    - message dict with content/reasoning_content (Qwen/LM Studio)
    - plain text string (standard)
    """
    if isinstance(response_data, dict):
        ...
        # For Qwen with enable_thinking=False, content has the answer
        ...
        if text.startswith('Thinking Process:'):
            text = text[len('Thinking Process:'):].strip()
        
        return text.strip() if text else ''
    
    # String case - try to extract after think tags
...
def run_benchmark(model_key, model_name):
    print(f'\n{"="*60}')
    print(f'BENCHMARK V2: {model_name}')
    print(f'{"="*60}')
    ...
    for i, args in enumerate(prompts):
        ...
        # Add enable_thinking: False for reasoning models like Qwen
        payload = {
            'model': model_key, 
            'messages': [
        ...
        try:
            ...
            if r.status_code == 200:
                response_data = r.json()['choices'][0]['message']
                text = extract_response(response_data)
...
if __name__ == '__main__':
    ...
    for model_key, model_name in MODELS:
        result = run_benchmark(model_key, model_name)
    ...
    print('='*60)
    print(f'{"Modelo":<25} {"Time":<8} {"TTFT":<8} {"Weighted":<10}')
    print('-'*60)
    for r in all_results:
        print(f'{r["model"]:<25} {r["time"]:<8.1f} {r["avg_ttft"]:<8.0f}ms {r["weighted"]:<10.1f}%')
    
    print('\n--- By Level ---')
    print(f'{"Modelo":<25} ' + ' '.join([f'L{l}' for l in range(1, 9)]))
    print('-'*60)
    for r in all_results:
        row = f'{r["model"]:<25}'
        for l in range(1, 9):
            score = r['by_level'].get(f'L{l}', 0)
            row += f' {score:5.1f}%'
        print(row)