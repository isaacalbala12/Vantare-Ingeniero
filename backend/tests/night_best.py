#!/usr/bin/env python3
"""Test verbose prompts similar to original benchmark vs current concise ones"""
import sys
...
# Test the ORIGINAL benchmark prompt + improved versions
PROMPTS = [
    # Original benchmark (verbose style)
    ('ORIG', 'Eres un ingeniero de carrera. Maximo 2-3 frases. Estilo radio.'),
    # Original + extraction directive
    ('ORIG+EXT', 'Eres un ingeniero de carrera. Maximo 2-3 frases. Estilo radio. Extrae datos de telemetria: posicion, vuelta, combustible, gap. Da solo los valores numericos.'),
    # Original + hybrid
    ('ORIG+HY', 'Eres ingeniero de carrera. Estilo radio, maximo 2-3 frases. Para preguntas de datos: extrae numeros. Para preguntas de accion: da orden breve. Sé conciso.'),
    # Concise (best from earlier)
...
async def run():
    ...
    for pname in all_results:
        ...
        avg = (avg_mm + avg_sf) / 2
        print(f'{pname}: MM={avg_mm:.1f}% SF={avg_sf:.1f}% AVG={avg:.1f}%')
