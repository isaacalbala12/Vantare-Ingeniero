#!/usr/bin/env python3
"""Full 22-question benchmark with top 3 prompts, smart extraction"""
import sys
...
async def run():
    ...
    for pname, sys_prompt in PROMPTS:
        ...
        for i, item in enumerate(Q):
            ...
            by_level[lvl].append((mm_scores[i], sf_scores[i]))
        
        level_str = ' '.join(f'{lv}:{sum(s)/len(s)*100:.0f}' for lv, s in sorted(by_level.items()))
        avg = (mm_pct + sf_pct) / 2
