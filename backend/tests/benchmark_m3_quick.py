...
Q = [
    ...
    ('L7', 'Fuel 5.2L en vuelta 35 de 38. Puedo hacer las 3?', ['3.4', '0', 'no', 'critico']),
]
...
print('MiniMax-M3 Quick Benchmark (8q, 8000 tokens)\n' + '='*50)
t0 = time.time()
total = 0

for i, item in enumerate(Q):
    lvl, q, kw = item[0], item[1], item[2]
    ...
    if r.status_code == 200:
        ...
        s = sc(text, kw)
        total += s
        print(f'[{lvl}] {s*100:.0f}% | "{text[:80]}"')
    else:
        print(f'[{lvl}] ERR {r.status_code}')

print(f'\nM3 SCORE: {total/len(Q)*100:.1f}% ({time.time()-t0:.1f}s)')
