#!/usr/bin/env python3
"""Test mixed: MM with NEW2, SF with ORIGINAL benchmark prompt"""
import sys
...
# The original benchmark uses this for SF (from benchmark_compare_cloud.py)
ORIG_SF = 'Eres un ingeniero de carrera. Maximo 2-3 frases. Estilo radio.'
...
async def call(client, url, model, key, sys, content, is_sf):
    payload = {'model': model, 'messages': [{'role': 'system', 'content': sys}, {'role': 'user', 'content': content}], 'max_tokens': 500, 'temperature': 0.1}
...
async def run():
    configs = [
        ('MM-NEW2', 'minimaxi', 'MiniMax-M2.7', M, NEW2, False),
        ('SF-ORIG', 'stepfun', 'step-3.7-flash', S, ORIG_SF, True),
        ('SF-NEW2', 'stepfun', 'step-3.7-flash', S, NEW2, True),
        ('MM-ORIG', 'minimaxi', 'MiniMax-M2.7', M, ORIG_SF, False),
    ]
    ...
    for run_i in range(3):
        ...
        for cfg_name, api, model, key, sys_prompt, is_sf in configs:
            ...
            async with httpx.AsyncClient() as client:
                ...
                for item in Q:
                    ...
                    tasks.append(call(client, base_url, model, key, sys_prompt, content, is_sf))
            ...
            all_results[cfg_name].append(pct)
            print(f'  {cfg_name}: {pct:.1f}%')
    
    print(f'\nDone in {time.time()-t0:.1f}s\n')
    print('=== FINAL RESULTS (3 runs) ===')
    for cfg_name in all_results:
        runs = all_results[cfg_name]
        avg = sum(runs) / len(runs)
        print(f'{cfg_name}: avg={avg:.1f}% runs={[f"{r:.1f}" for r in runs]}')
    
    # Best combos
    mm_avg = sum(all_results['MM-NEW2']) / 3
    sf_orig_avg = sum(all_results['SF-ORIG']) / 3
    sf_new_avg = sum(all_results['SF-NEW2']) / 3
    orig_mm = sum(all_results['MM-ORIG']) / 3
    
    print(f'\n--- ANALYSIS ---')
    print(f'MM NEW2: {mm_avg:.1f}% vs MM ORIG: {orig_mm:.1f}%')
    print(f'SF NEW2: {sf_new_avg:.1f}% vs SF ORIG: {sf_orig_avg:.1f}%')
    print(f'\nCOMBO (MM-NEW2 + SF-ORIG): {(mm_avg + sf_orig_avg)/2:.1f}%')
    print(f'COMBO (MM-NEW2 + SF-NEW2): {(mm_avg + sf_new_avg)/2:.1f}%')
