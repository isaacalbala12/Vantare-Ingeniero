#!/usr/bin/env python3
"""Run the ACTUAL benchmark_compare_cloud.py with best new prompts for comparison"""
import sys
...
def extract_mm(msg):
    """Same as benchmark_compare_cloud.py"""
...
# FULL benchmark questions (same as benchmark_compare_cloud.py)
Q = [
...
# Test original vs best new
PROMPTS = [
    ('ORIG', 'Eres un ingeniero de carrera. Maximo 2-3 frases. Estilo radio.'),
    ('NEW1', 'Short direct answer. For numbers: extract from telemetry. For advice: short action word. Max 5 words. No sentences.'),
    ...
    ('NEW3', 'Race engineer. Data: extract numbers (P3 L10 42.3). Action: short verb (Entrar, Gestionar). Max 3 words. No sentences. Examples: P3 L10 42.3 | Entrar boxes.'),
]
...
async def run():
    ...
    print(f'\nDone in {time.time()-t0:.1f}s ({len(Q)} questions x {len(PROMPTS)} prompts)')
