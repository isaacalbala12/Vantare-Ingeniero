#!/usr/bin/env python3
"""Test MiniMax-M3.0 with ORIG and NEW2 prompts"""
import sys
...
for run_i in range(3):
    for name, sys_prompt in [('ORIG', ORIG_SYS), ('NEW', NEW_SYS)]:
        total = 0
        by_level = {}
        
        print(f'\n--- MiniMax-M3 Run {run_i+1}/3 [{name}] ---')
        ...
        for i, item in enumerate(ORIG_Q):
            ...
            payload = {'model': 'MiniMax-M3.0', 'messages': [{'role': 'system', 'content': sys_prompt}, {'role': 'user', 'content': content}], 'max_tokens': 500, 'temperature': 0.1}
            
            r = httpx.post('https://api.minimaxi.chat/v1/chat/completions', headers=h, json=payload, timeout=120)
