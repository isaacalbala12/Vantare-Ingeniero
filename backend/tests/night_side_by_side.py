#!/usr/bin/env python3
"""Side-by-side test: Original prompt vs NEW2 on SAME questions"""
import sys
...
async def call(client, url, model, key, sys, content, is_sf, temp):
    payload = {'model': model, 'messages': [{'role': 'system', 'content': sys}, {'role': 'user', 'content': content}], 'max_tokens': 500, 'temperature': temp}
    if is_sf: payload['thinking'] = {'type': 'off'}
    h = {'Authorization': f'Bearer {key}', 'Content-Type': 'application/json'}
    try:
        r = await client.post(url + '/chat/completions', headers=h, json=payload, timeout=60.0)
...
async def run():
    configs = [
        ('MM-ORIG-t01', 'minimaxi', 'MiniMax-M2.7', M, ORIG_SYS, False, 0.1),
        ('MM-NEW2-t01', 'minimaxi', 'MiniMax-M2.7', M, NEW_SYS, False, 0.1),
        ('MM-NEW2-t03', 'minimaxi', 'MiniMax-M2.7', M, NEW_SYS, False, 0.3),
        ('SF-ORIG-t01', 'stepfun', 'step-3.7-flash', S, ORIG_SYS, True, 0.1),
        ('SF-NEW2-t01', 'stepfun', 'step-3.7-flash', S, NEW_SYS, True, 0.1),
    ]
    
    results = {}
    t0 = time.time()
    
    for cfg_name, api, model, key, sys_prompt, is_sf, temp in configs:
        base_url = 'https://api.minimaxi.chat/v1' if api == 'minimaxi' else 'https://api.stepfun.ai/step_plan/v1'
        scores = []
        ...
        async with httpx.AsyncClient() as client:
            ...
            for item in Q:
                ...
                tasks.append(call(client, base_url, model, key, sys_prompt, content, is_sf, temp))
            msgs = await asyncio.gather(*tasks)
            
            for i, item in enumerate(Q):
                kw = item[3]
                extractor = extract_mm if api == 'minimaxi' else extract_sf
                text = extractor(msgs[i]) if msgs[i] else ''
                scores.append(sc(text, kw))
        
        pct = sum(scores) / len(Q) * 100
        results[cfg_name] = pct
        print(f'{cfg_name}: {pct:.1f}%')
    
    print(f'\nDone in {time.time()-t0:.1f}s')
    print(f'\n--- SUMMARY ---')
    print(f'MM Original: {results["MM-ORIG-t01"]:.1f}%')
    print(f'MM NEW2 t01: {results["MM-NEW2-t01"]:.1f}%')
    print(f'MM NEW2 t03: {results["MM-NEW2-t03"]:.1f}%')
    print(f'SF Original: {results["SF-ORIG-t01"]:.1f}%')
    print(f'SF NEW2: {results["SF-NEW2-t01"]:.1f}%')
    
    # Best combo
    best_mm = max([k for k in results if 'MM' in k], key=lambda k: results[k])
    best_sf = max([k for k in results if 'SF' in k], key=lambda k: results[k])
    combined = (results[best_mm] + results[best_sf]) / 2
    print(f'\nBEST PAIR: {best_mm}={results[best_mm]:.1f}% + {best_sf}={results[best_sf]:.1f}% = {combined:.1f}%')
