...
print('MiniMax-M3 FULL Benchmark (22 questions)\n' + '='*60)
t0 = time.time()
total = 0
by_level = {}
...
for i, item in enumerate(Q):
    ...
    if extra: content = f'### TELEMETRIA ###\n{TICKERS.get(tid, "")}\n### HISTORICO ###\n{extra}\n### PREGUNTA ###\n{q}'

    r = httpx.post('https://api.minimaxi.chat/v1/chat/completions', headers=h,
        json={'model': 'MiniMax-M3', 'messages': [{'role': 'system', 'content': SYS}, {'role': 'user', 'content': content}], 'max_tokens': 500, 'temperature': 0.1}, timeout=60)
    ...
    if r.status_code == 200:
        ...
        if lvl not in by_level: by_level[lvl] = []
        by_level[lvl].append(0)

pct = total / len(Q) * 100
print(f'\nTOTAL: {pct:.1f}% ({time.time()-t0:.1f}s)')
print('\nBy Level:')
