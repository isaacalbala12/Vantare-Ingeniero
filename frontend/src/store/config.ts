...
const loadSavedConfig = (): AppConfig => {
	try {
		const saved = localStorage.getItem("vantare_config");
		if (saved) {
			const parsed = JSON.parse(saved);
			const schemaVersion = parsed.configSchemaVersion ?? 1;
			...
			const config: AppConfig = {
				...
				wakeWordEnabled: legacy ? false : (parsed.wakeWordEnabled ?? false),
				...
				spotterHoldRepeatS:
					parsed.spotterHoldRepeatS ?? parsed.spotterOverlapDelayS ?? 3.0,
				spotterGapFrequencyS: parsed.spotterGapFrequencyS ?? 30,
				spotterCarLengthM: parsed.spotterCarLengthM ?? 4.5,
				spotterMinSpeedMs: migrateV3 ? 5.0 : (parsed.spotterMinSpeedMs ?? 5.0),
				spotterRaceStartDelayS: migrateV3
					? 3.0
					: (parsed.spotterRaceStartDelayS ?? 3.0),
				brakingZonesMute: parsed.brakingZonesMute ?? false,
				speakOnlyWhenSpokenTo: legacy
					? true
					: (parsed.speakOnlyWhenSpokenTo ?? true),
