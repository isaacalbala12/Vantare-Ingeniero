...
const SECTION_KEY: Record<HubSection, Parameters<typeof t>[1]> = {
  inicio: "home",
  ingeniero: "engineer",
  spotter: "spotter",
  audio: "audio",
...
export function HubSidebar({ active, onNavigate }: HubSidebarProps) {
  ...
  return (
    <aside className="w-[260px] shrink-0 bg-hub-sidebar border-r border-hub-border flex flex-col">
      <div className="px-6 py-5 border-b border-hub-border">
        ...
        <div className="text-[10px] uppercase tracking-[0.35em] text-a1-accent mt-1">{t(uiLanguage, "aiEngineer")}</div>
      </div>
      <nav className="flex-1 p-3 flex flex-col gap-1">
        {(Object.keys(SECTION_KEY) as HubSection[]).map((id) => {
          ...
          return (
            ...
            >
              <span className={isActive ? "text-a1-accent" : "text-a1-text-muted"}>{ICONS[id]}</span>
              {t(uiLanguage, SECTION_KEY[id])}
            </button>
          );
        })}
      </nav>
